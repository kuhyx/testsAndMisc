...
| `billsplit/`           | Receipt bill-splitting Flutter app — people, groups, per-item assignment, e-paragon import and xlsx export (Dart, 100 % line coverage gate)                                                                |
| `linux_configuration/` | Arch Linux setup, i3 config, system maintenance scripts                                                                                                                                                    |
| `focus_owner/`         | Device Owner app enforcing focus mode on the unrooted Pixel 6a — geofence + night curfew, hides apps the phone cannot un-hide (Dart/Kotlin)                                                                |
| `phone_focus_mode/`    | GPS-based Android focus enforcer (rooted-era scripts; `config.sh` is still the policy source of truth for `focus_owner/`)                                                                                  |
| `reverse_survivors/`   | Reverse Survivors — React/Vite game where you play the horde against an AI survivor (TypeScript, 100 % coverage gate)                                                                                      |
