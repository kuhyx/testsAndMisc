...
# CLEAN and TOTAL_RECLAIMABLE must exist before the helpers are sourced:
# `confirm` reads CLEAN and `report` accumulates into TOTAL_RECLAIMABLE.
CLEAN=false
if [[ "${1:-}" == "--clean" ]]; then
	CLEAN=true
fi
...
# shellcheck source=./lib/disk_cleanup_common.sh
source "$SCRIPT_DIR/lib/disk_cleanup_common.sh"

# ───────────────────── checks ─────────────────────

printf "\n%s=== Disk Cleanup Analysis ===%s\n\n" "${BOLD}" "${RESET}"
...
if ((pacman_cache_size > 0)); then
	if command -v paccache &>/dev/null; then
		pkg_count=$(find /var/cache/pacman/pkg/ -maxdepth 1 -type f -name '*.pkg.tar*' 2>/dev/null | wc -l)
	...
	if ((old_archives_kb > 0)); then
		...
		if confirm "Delete old AUR archives (>30 days)?"; then
			find "$aur_dir" \
				...
				-o -name "*.zip" -o -name "*.tar.bz2" \) \
				-mtime +30 -delete
			printf "%s  ✓ Old AUR archives deleted%s\n" "${GREEN}" "${RESET}"
		fi
	fi
fi
