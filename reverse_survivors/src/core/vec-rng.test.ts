...
describe('vec', () => {
  ...
  it('builds a velocity toward a target with the requested speed', () => {
    const v = toward({ x: 0, y: 0 }, { x: 10, y: 0 }, 50)
    expect(v).toEqual({ x: 50, y: 0 })
  })

  it('returns zero velocity for coincident points', () => {
    expect(toward({ x: 5, y: 5 }, { x: 5, y: 5 }, 99)).toEqual({ x: 0, y: 0 })
  })
...
describe('rng', () => {
  it('is deterministic for a given seed', () => {
    ...
    expect([nextFloat(a), nextFloat(a), nextFloat(a)])
      .toEqual([nextFloat(b), nextFloat(b), nextFloat(b)])
  })
  ...
  it('pick is deterministic', () => {
    expect(pick(createRng(42), [10, 20, 30] as const))
      .toBe(pick(createRng(42), [10, 20, 30] as const))
  })
})
