...
def test_run_progress_loop_collects_every_segment(
    ...
    monkeypatch.setattr(fw.sys.stderr, "isatty", lambda: False)

    collected, info = fw._run_progress_loop(_args(), model, "in.wav", 3.0)
...
def test_run_progress_loop_tolerates_a_segment_without_an_end(
    ...
    collected, _ = fw._run_progress_loop(_args(), model, "in.wav", 10.0)
...
def test_run_progress_loop_stays_silent_with_no_progress(
    ...
    with caplog.at_level(logging.INFO):
        fw._run_progress_loop(_args(no_progress=True), model, "in.wav", 2.0)
...
def test_run_progress_loop_emits_progress_on_a_tty(
    ...
    model = _Model([_Segment(1.0), _Segment(2.0)])
    monkeypatch.setattr(fw.sys.stderr, "isatty", lambda: True)

    with caplog.at_level(logging.INFO):
        fw._run_progress_loop(_args(), model, "in.wav", 2.0)
...
def test_run_progress_loop_throttles_when_not_a_tty(
    ...
    monkeypatch.setattr(fw.time, "time", lambda: next(clock))

    with caplog.at_level(logging.INFO):
        fw._run_progress_loop(_args(), model, "in.wav", 10.0)
