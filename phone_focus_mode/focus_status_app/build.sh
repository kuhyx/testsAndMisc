...
_bt_dirs=("$SDK"/build-tools/*/)
BUILD_TOOLS_DIR="$(printf '%s\n' "${_bt_dirs[@]}" | sort -V | tail -1 | sed 's:/$::')"
# Pick highest-numbered platform.
_plat_dirs=("$SDK"/platforms/android-*/)
PLATFORM_DIR="$(printf '%s\n' "${_plat_dirs[@]}" | sort -V | tail -1 | sed 's:/$::')"
PLATFORM_JAR="$PLATFORM_DIR/android.jar"

[ -d "$BUILD_TOOLS_DIR" ] || {
	echo "ERROR: build-tools not found under $SDK" >&2
	exit 1
}
[ -f "$PLATFORM_JAR" ] || {
	echo "ERROR: android.jar not found at $PLATFORM_JAR" >&2
	exit 1
}
