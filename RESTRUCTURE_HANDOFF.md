# testsAndMisc restructure — the only handoff file

**This is the whole handoff. There is no second file to find.**

- **To start work:** `/clear`, then paste from the line `## Prompt — paste
from here` down to `## Reference`. Or just paste the whole file — that works
  too, and is the safer default.
- **Superseded:** the old `NEXT_SESSION_PROMPT.md` and
  `NEXT_SESSION_RESTRUCTURE.md` were merged into this file on 2026-08-23.
- **Unrelated task:** `INSTALLER_FIX_TASK.md` is a different job
  (`install_core_system.sh` on a fresh machine). Not part of the restructure.
...
then `digital_wellbeing/`. Everything you need is in this file — the
reference sections below the prompt are part of it. Read "Traps already paid
for" before touching anything; every line in it cost a real failure.
...
## Reference — stop pasting here (the agent reads the rest itself)
