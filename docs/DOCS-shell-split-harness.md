# Shell split verification: the harness

Split out of `docs/shell-split-verification.md` when that file hit the 250-line
cap. That file covers **what makes a green split wrong**; this one covers the
tooling that catches it — the trace harness, its `--prefix` mode, the
regression fixtures, and two traps that have each cost a revert.

Read `docs/shell-split-verification.md` first: the hazards there decide whether
the harness can prove anything about your target at all.
